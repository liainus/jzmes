...
imp.filter.add('http://WebXml.com.cn/')
@Process.route('/SAP_MailInfos', methods=['GET', 'POST'])
def SAP_MailInfos():
    '''SAP物料主数据同步接口'''
    if request.method == 'GET':
        data = request.values
        try:
            ...
            if len(jsonstr) > 1:
                dic = {}
                rid = round(time.time() * 1000)
                data_ma = {"RID": rid, "WERKS": "JZLT", "MTART": "Z001", "CDATE": datetime.datetime.now().strftime("%Y%m%d"),
                       "EDATE": datetime.datetime.now().strftime("%Y%m%d"), "SYSID": "MES", "OPDAT": datetime.datetime.now().strftime("%Y%m%d"),
                ...
                jsondic["OPERATION"] = ""
                jsondic["DESCRIPTION"] = ""
                jsondic["DATA"] = data_ma
                data_json = json.dumps(jsondic, cls=AlchemyEncoder, ensure_ascii=False)
                print(data_json)
                headers = {'Content-Type': 'application/soap+xml; charset="UTF-8"'}
                xml = Raw("""<![CDATA[<Request><soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:urn="urn:sap-com:document:sap:rfc:functions">
                        <soapenv:Header/><soapenv:Body><urn:Z_PP_MES_INTF>
                        <IV_INPUT>"""+data_json+"""</IV_INPUT>
                        <IV_INTF_ID>PP1005</IV_INTF_ID>
                        <IV_SYSID>MES</IV_SYSID>
                        </urn:Z_PP_MES_INTF></soapenv:Body></soapenv:Envelope></Request>]]>""")
                auth = HttpAuthenticated(username="INTF", password="Hrjz1234")
                url = "http://10.136.0.18:8001/sap/bc/srt/wsdl/flv_10002A111AD1/bndg_url/sap/bc/srt/rfc/sap/zws_mes_intf/300/zws_mes_intf/zbd_mes_intf?sap-client=300&sap-language=ZH"
                client = Client(url, transport=auth)
                response = client.service.Z_PP_MES_INTF('', 'PP1005', 'MES')
                # response = client.service.Z_PP_MES_INTF(__inject={'IV_INPUT': data_json,'IV_INTF_ID': 'PP1005','IV_SYSID': 'MES',})#, headers=headers, faults=False, timeout=15, doctor=ImportDoctor(imp)
                # import requests
                # auth = {"Username":"INTF", "Password":"Hrjz1234"}
                # response = requests.post("http://10.136.0.18:8001/sap/bc/srt/rfc/sap/zws_mes_intf/300/zws_mes_intf/zbd_mes_intf", Z_PP_MES_INTF=xml, json=auth, headers=headers)
                # print(response)

                # soap_rep = getattr(client.service, 'Z_PP_MES_INTF')(IV_INPUT=data_json, IV_INTF_ID="PP1005",
                #                                                     IV_SYSID="MES")
                print(response)
                db_session.commit()
...
@Process.route('/SAP_OrderSynchonizes', methods=['GET', 'POST'])
def SAP_OrderSynchonizes():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if len(jsonstr) > 10:
                jsonnumber = re.findall(r"\d+\.?\d*", jsonstr)
                dic = {"RID": "", "AFPO-DWERK":"", "AFPO-DAUAT": "102", "AFPO-MATNR": data.get("StartTime"),
                         "AFPO-VERID": data.get("EndTime")}
                jsondic = json.dumps(dic, cls=AlchemyEncoder, ensure_ascii=False)
                client = Client(Model.Global.SAPurl)
                ret = client.service.Mes_Interface("billload", jsondic)
                if ret[0] != "SUCCESS":
                    return json.dumps("调用SAP订单信息同步接口报错！" + ret[1])
                db_session.commit()
                return 'OK'
        except Exception as e:
            print("调用SAP订单信息同步接口口报错！")
            return json.dumps("调用SAP订单信息同步接口报错！")
aa = "http://127.0.0.1:5001/?wsdl"
@Process.route('/aasd', methods=['GET', 'POST'])
def aasd():
    '''SAP订单信息同步接口'''
    if request.method == 'GET':
        data = request.values
        try:
            dic = {}
            rid = round(time.time() * 1000)
            data_ma = {"RID": rid, "WERKS": "JZLT", "MTART": "Z001",
                       "CDATE": datetime.datetime.now().strftime("%Y%m%d"),
                       "EDATE": datetime.datetime.now().strftime("%Y%m%d"), "SYSID": "MES",
            ...
            div = {}
            dii["IV_SYSID"] = "MES"
            div["IV_INTF_ID"] = "PP1005"
            data_json = json.dumps(jsondic, cls=AlchemyEncoder, ensure_ascii=False)
            dic["IV_INPUT"] = ""
            jsondic = json.dumps(dic, cls=AlchemyEncoder, ensure_ascii=False)
            client = Client(aa)
            ret = client.service.WMS_OrderStatus(json_data="2132",name="1")
            if ret[0] != "SUCCESS":
                return json.dumps("调用SAP订单信息同步接口报错！" + ret[1])
            db_session.commit()
            return 'OK'
